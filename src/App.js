...
import OpenAiService from "./aiservicehelpers/OpenAiService.js";
import { Server } from "socket.io";
import * as http from "http";
import Queue from "queue";
import JobList from "./JobList.js";
import GrokAIService from "./aiservicehelpers/GrokAiService.js";
...
export default class App {
  ...
  constructor() {
    ...
    this.#AI_CHOICE = getConfigVariable("AI_CHOICE", "OPENAI");
  }

  async run() {
    this.#firefly = new FireflyService();

    switch (this.#AI_CHOICE) {
      case "GROKAI":
        this.#aiService = new GrokAIService();
        break;
      default:
  ...
  #handleWebhook(req, res) {
    ...
    if (req.body.content.transactions[0].type !== "withdrawal") {
      throw new WebhookException(
        "content.transactions[0].type has to be 'withdrawal'. Transaction will be ignored."
      );
    }
